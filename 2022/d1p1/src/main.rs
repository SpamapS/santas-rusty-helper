...
This list represents the Calories of the food carried by five Elves:
    ...
    The second Elf is carrying one food item with 4000 Calories. The third Elf is carrying food
    with 5000 and 6000 Calories, a total of 11000 Calories. The fourth Elf is carrying food with
    7000, 8000, and 9000 Calories, a total of 24000 Calories. The fifth Elf is carrying one food
    item with 10000 Calories.
...
fn parse_calories(input: &str) -> Vec<Vec<u32>> {
    ...
    loop {
        match lines.next() {
            None => {
                parsed.push(elf_inventory);
                break;
            },
            Some(line) => {
                if line.len() == 0 {
                    parsed.push(elf_inventory.clone());
                    elf_inventory.clear();
                } else {
                    let calories: u32 = line.parse().unwrap();
                    elf_inventory.push(calories);
                }
            }
        }
    }
...
fn test_parse_calories() {
    let parsed_test: Vec<Vec<u32>> = vec!(
    ...
    assert!(parse_calories(TEST_INPUT) == parsed_test);
}

fn hungriest_elf(elf_inventory: Vec<Vec<u32>>) -> u32 {
    return elf_inventory.iter()
        .map(|this_elf| this_elf.iter().sum())
        .max().unwrap();
}

#[test]
fn test_hungriest_elf() {
